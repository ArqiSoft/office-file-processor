...
//    }
    ...
    @Bean
    BlockingQueue<ConvertToPdf> getConvertToPdfProcessingQueue(
            @Value("${QUEUE_PREFETCH_SIZE:15}") Integer prefetchSize) {
        LOGGER.debug("QUEUE_PREFETCH_SIZE is set to {}", prefetchSize);
        return new ArrayBlockingQueue<ConvertToPdf>(prefetchSize);
    }
    
    @Bean
    BlockingQueue<ExtractMeta> getExtractMetaProcessingQueue(
            @Value("${QUEUE_PREFETCH_SIZE:15}") Integer prefetchSize) {
        LOGGER.debug("QUEUE_PREFETCH_SIZE is set to {}", prefetchSize);
        return new ArrayBlockingQueue<ExtractMeta>(prefetchSize);
    }
}
